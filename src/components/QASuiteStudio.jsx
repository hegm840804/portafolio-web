...
export default function QASuiteStudio({ onOpenContact }) {
  ...
  // =========================================================================
  // 1. GENERADOR DE MATRIZ CON SELECTOR INTERACTIVO (JR, MED, SR) Y MÃƒÆ’Ã¢â‚¬Â°TRICAS
  // =========================================================================
  ...
  const [requerimiento, setRequerimiento] = useState({
    idHU: 'HU-SPEI-104',
    prefijoID: 'SPEI',
    titulo: 'MÃƒÆ’Ã‚Â³dulo de Transferencias Interbancarias SPEI en Tiempo Real',
    ...
    origen: 'EspecificaciÃƒÆ’Ã‚Â³n Core Bancario / FinTech',
    criterios: [
      'La cuenta CLABE debe contener exactamente 18 dÃƒÆ’Ã‚Â­gitos numÃƒÆ’Ã‚Â©ricos vÃƒÆ’Ã‚Â¡lidos bajo algoritmo MÃƒÆ’Ã‚Â³dulo 10 Banxico.',
      'El monto a transferir debe ser mayor a $0.00 y menor o igual al saldo lÃƒÆ’Ã‚Â­quido disponible.',
      'Toda transacciÃƒÆ’Ã‚Â³n aprobada debe generar un folio de rastreo CEP ÃƒÆ’Ã‚Âºnico y persistir en base de datos.'
    ]
  });
  ...
  const manejarSubidaReq = (e) => {
    ...
    if (file) {
      ...
      setRequerimiento({
        ...
        descripcion: `EspecificaciÃƒÆ’Ã‚Â³n cargada desde ${file.name} (${(file.size / 1024).toFixed(1)} KB). IDs generados con trazabilidad [${prefijoDinamico}].`,
        origen: 'Archivo local (' + file.name + ')',
        criterios: [
          'ValidaciÃƒÆ’Ã‚Â³n de reglas extraÃƒÆ’Ã‚Â­das del documento.',
  ...
  // GENERADOR DINÃƒÆ’Ã‚ÂMICO SEGÃƒÆ’Ã…Â¡N NIVEL (JR: ~30 | MED: ~75 | SR: ~120)
  const generarSuitePorNivel = (prefijo, nivel) => {
    let distribucion = [
      ...
      { code: 'SQL', name: 'AuditorÃƒÆ’Ã‚Â­a SQL', casos: 2 }
    ]; // Junior ~30 casos
    ...
    if (nivel === 'semi') {
      distribucion = [
        ...
        { code: 'SQL', name: 'AuditorÃƒÆ’Ã‚Â­a SQL', casos: 7 }
      ...
      distribucion = [
        ...
        { code: 'SQL', name: 'AuditorÃƒÆ’Ã‚Â­a SQL', casos: 15 }
      ]; // Senior ~125 casos
    }
    ...
    distribucion.forEach(seccion => {
      for (let i = 1; i <= seccion.casos; i++) {
        ...
        suite.push({
          ...
          pasosArray: [
            { num: 1, accion: `Configurar entorno para prueba ${seccion.name} #${i} bajo requerimiento ${requerimiento.idHU}.`, data: `Param_${i}=Activo | Auth=JWT_Valid`, esperado: 'Entorno listo para validaciÃƒÆ’Ã‚Â³n.' },
            { num: 2, accion: `Ejecutar validaciÃƒÆ’Ã‚Â³n de ${seccion.name.toLowerCase()} en el mÃƒÆ’Ã‚Â³dulo de negocio.`, data: `Vector_Test=${seccion.code}_${i}`, esperado: 'Respuesta conforme a especificaciÃƒÆ’Ã‚Â³n tÃƒÆ’Ã‚Â©cnica.' },
            { num: 3, accion: 'Verificar registros de auditorÃƒÆ’Ã‚Â­a y persistencia en base de datos.', data: `Query: SELECT * FROM logs WHERE ref = '${idCaso}'`, esperado: 'Persistencia inmutable verificada.' }
          ],
          valores: {
            'ID_Caso': idCaso,
            'Modulo_Core': `${prefijo} / MÃƒÆ’Ã‚Â³dulo ${seccion.name}`,
            'Requerimiento_Asociado': requerimiento.idHU,
            'Descripcion_Escenario': `ValidaciÃƒÆ’Ã‚Â³n (${seccion.name}) #${i} basada en la especificaciÃƒÆ’Ã‚Â³n del requerimiento ${requerimiento.idHU}.`,
            'Tipo_Validacion': `QA ${nivel.toUpperCase()} - ${seccion.name}`,
            'Precondiciones': `Servicios core activos y parÃƒÆ’Ã‚Â¡metros cargados para ${idCaso}.`,
            'Pasos_Detallados': `1. Configurar datos ${i}.\n2. Ejecutar transacciÃƒÆ’Ã‚Â³n.\n3. Validar BD.`,
            'Valores_Entrada_TestData': `Dataset_${seccion.code}_${i}`,
            'Comportamiento_Esperado': `El sistema cumple con la regla de aceptaciÃƒÆ’Ã‚Â³n para ${seccion.name}.`,
            'Severidad': i <= 2 ? 'CrÃƒÆ’Ã‚Â­tica' : 'Alta',
            'Estado': 'Listo para EjecuciÃƒÆ’Ã‚Â³n'
          }
        });
      }
    });
  ...
  // =========================================================================
  // 2. MÃƒÆ’Ã¢â‚¬Å“DULO N8N & WEBHOOKS
  // =========================================================================
  ...
  const [payloadJson, setPayloadJson] = useState(JSON.stringify({
    ...
    cliente: {
      ...
      empresa: "Servicios de AutomatizaciÃƒÆ’Ã‚Â³n & QA",
  ...
  const cambiarEvento = (e) => {
    ...
    if (val === 'lead') {
      ...
      setPayloadJson(JSON.stringify({
        ...
        severidad: "CrÃƒÆ’Ã‚Â­tica",
        moduloAfectado: "SPEI / Core Transferencias",
        descripcion: "Fallo en algoritmo de dÃƒÆ’Ã‚Â­gito verificador CLABE",
        ambiente: "Staging / Pre-ProducciÃƒÆ’Ã‚Â³n",
  ...
  const ejecutarFlujoN8n = () => {
    ...
    setTimeout(() => {
      ...
      setLogSalida({
        status: 200,
        mensaje: "Evento '" + (parsed.evento || 'HTTP_EVENT') + "' procesado exitosamente por la canalizaciÃƒÆ’Ã‚Â³n.",
  ...
  return (
    <section id="automatizaciones" className="max-w-6xl mx-auto px-4 py-16 w-full scroll-mt-24">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
          <div className="relative w-full max-w-2xl bg-slate-900 border border-cyan-500/60 rounded-3xl p-6 sm:p-8 shadow-2xl text-slate-100 space-y-5 max-h-[92vh] overflow-y-auto">
            ...
            <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl text-xs space-y-1">
              <span className="font-bold text-cyan-300 block text-[11px]">ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¯ Precondiciones:</span>
            ...
            {!veredictoFinal ? (
              <div className="bg-slate-950 border border-cyan-500/40 rounded-2xl p-5 space-y-4 shadow-xl">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <span className="text-xs font-bold text-cyan-400 uppercase tracking-wider">
                    ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ¢â‚¬Â° Paso {casoEnEjecucion.pasosArray[pasoActualIdx].num} en Progreso
                  </span>
                </div>

                <div className="space-y-3 text-xs">
                  <div>
                    <span className="text-slate-400 font-semibold block text-[11px]">ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ AcciÃƒÆ’Ã‚Â³n:</span>
                    <p className="text-white font-medium text-sm mt-0.5">{casoEnEjecucion.pasosArray[pasoActualIdx].accion}</p>
                  </div>
                  <div className="p-3 bg-slate-900 rounded-xl border border-slate-800">
                    <span className="text-cyan-300 font-semibold block text-[11px]">ÃƒÂ°Ã…Â¸Ã‚Â§Ã‚Âª Datos de Prueba:</span>
                  ...
                  <div className="p-3 bg-emerald-950/40 rounded-xl border border-emerald-900/60">
                    <span className="text-emerald-400 font-semibold block text-[11px]">ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Resultado Esperado:</span>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  >
                    ÃƒÂ¢Ã‚ÂÃ…â€™ Registrar Falla (Fail)
                  </button>
                  <button
                  ...
                  >
                    ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Paso Exitoso (Pass) ÃƒÂ¢Ã…Â¾Ã¢â‚¬Â
                  </button>
                </div>
              </div>
            ) : (
              <div className={`p-6 rounded-2xl border text-center space-y-4 ${
                ...
                <div className="text-4xl">{veredictoFinal === 'PASSED' ? 'ÃƒÂ°Ã…Â¸Ã‚ÂÃ¢â‚¬Â ' : 'ÃƒÂ°Ã…Â¸Ã‚ÂÃ¢â‚¬Âº'}</div>
                <div>
                  <h4 className="text-lg font-extrabold">
                    {veredictoFinal === 'PASSED' ? 'Ãƒâ€šÃ‚Â¡Caso Ejecutado con ÃƒÆ’Ã¢â‚¬Â°xito (PASS)!' : 'Defecto Detectado (FAIL)'}
                  </h4>
                </div>
                <div className="flex justify-center gap-3">
                  <button onClick={reiniciarEjecucion} className="bg-slate-800 text-white text-xs px-4 py-2 rounded-xl cursor-pointer">ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Å¾ Re-ejecutar</button>
      ...
      {/* SELECTOR DE PESTAÃƒÆ’Ã¢â‚¬ËœAS PRINCIPALES */}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        {/* PESTAÃƒÆ’Ã¢â‚¬ËœA 1: MATRIZ QA CON LOS 3 PASOS */}
        {pestanaActiva === 'matriz' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 border-b border-slate-800 pb-5">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold">Archivo 3: Suite QA & MÃƒÆ’Ã‚Â©tricas</span>
                </div>
              ...
              <div className="space-y-5 text-xs">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-3">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400">Sube la especificaciÃƒÆ’Ã‚Â³n o historia de usuario que deseas probar.</p>
                  </div>
                </div>
                ...
                <div className="bg-slate-950 border-2 border-dashed border-slate-700 hover:border-emerald-500 rounded-2xl p-6 text-center space-y-3 transition">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-1">
                      Soporta: ImÃƒÆ’Ã‚Â¡genes (.png, .jpg), Documentos (.pdf, .docx), Excel (.xlsx) o Texto (.txt)
                    </p>
                  </div>
                  ...
                  {vistaPreviaReqImg && (
                    <div className="max-w-xs mx-auto p-2 bg-slate-900 rounded-xl border border-slate-800">
                      ...
                      <span className="text-[10px] text-emerald-400 block mt-1">ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Imagen de requerimiento cargada</span>
                    </div>
                  )}
            ...
            <div className="mt-4 p-4 bg-gray-900 border border-gray-700 rounded-lg">
              <h3 className="text-sm font-semibold text-gray-200 mb-2">Ã¢Å“ÂÃ¯Â¸Â Redactar Requerimiento / Proyecto Manualmente</h3>
              <textarea
                ...
                placeholder="Escribe aquÃƒÂ­ los detalles de tu flujo o historia de usuario si no cuentas con un archivo..."
              ...
              <h3 className="text-sm font-semibold text-gray-200 mb-2">Ã°Å¸â€œÅ’ Notas u Observaciones (Opcional)</h3>
              <textarea
        ...
        {/* PESTAÃƒÆ’Ã¢â‚¬ËœA 2: ORQUESTADOR N8N */}
        {pestanaActiva === 'n8n' && (
          <div className="space-y-6">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800 pb-6 mb-8">
              <div>
                <div className="inline-flex items-center gap-2 bg-cyan-950/80 border border-cyan-500/40 px-3 py-1 rounded-full text-xs font-semibold text-cyan-300 mb-2">
                  ...
                  <span>MÃƒÆ’Ã‚Â³dulo Funcional: AutomatizaciÃƒÆ’Ã‚Â³n con n8n</span>
                </div>
                <h2 className="text-2xl sm:text-3xl font-extrabold text-white">
                  OrquestaciÃƒÆ’Ã‚Â³n de Flujos & Webhooks en Vivo
                </h2>
                <p className="text-xs sm:text-sm text-slate-400 mt-1">
                  Prueba la recepciÃƒÆ’Ã‚Â³n de peticiones HTTP, validaciÃƒÆ’Ã‚Â³n de schemas JSON y despacho de alertas en tiempo real.
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  <option value="lead">ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¨ Captura de Lead / CotizaciÃƒÆ’Ã‚Â³n</option>
                  <option value="bug">ÃƒÂ°Ã…Â¸Ã‚ÂÃ¢â‚¬Âº Reporte de Defecto (Jira QA)</option>
                  <option value="api-health">ÃƒÂ°Ã…Â¸Ã‚Â©Ã‚Âº Health-Check API SPEI</option>
                </select>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-5 space-y-4">
                <div className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-3 text-xs">
                  <div>
                    <label className="block text-slate-400 font-semibold mb-1">MÃƒÆ’Ã‚Â©todo & Endpoint HTTP POST:</label>
                  ...
                  <div>
                    ...
                    ></textarea>
                    {errorJson && (
                      <p className="text-rose-400 text-[11px] font-semibold mt-1">ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â {errorJson}</p>
                    )}
                  </div>
                  ...
                  >
                    <span>{ejecutandoN8n ? 'ÃƒÂ¢Ã…Â¡Ã¢â€žÂ¢ÃƒÂ¯Ã‚Â¸Ã‚Â' : 'ÃƒÂ¢Ã…Â¡Ã‚Â¡'}</span>
                ...
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-2xl font-mono text-xs text-slate-300 space-y-2">
                  <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                    <span className="text-emerald-400 font-bold text-[11px]">
                      ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ HTTP {logSalida.status} OK ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Latencia: {logSalida.tiempoTotal}
                    </span>
