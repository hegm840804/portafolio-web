...
export default function QASuiteStudio({ onOpenContact }) {
  ...
  // =========================================================================
  // 1. GENERADOR DE MATRIZ CON SELECTOR INTERACTIVO (JR, MED, SR) Y MÃ‰TRICAS
  // =========================================================================
  ...
  const [requerimiento, setRequerimiento] = useState({
    idHU: 'HU-SPEI-104',
    prefijoID: 'SPEI',
    titulo: 'MÃ³dulo de Transferencias Interbancarias SPEI en Tiempo Real',
    ...
    origen: 'EspecificaciÃ³n Core Bancario / FinTech',
    criterios: [
      'La cuenta CLABE debe contener exactamente 18 dÃ­gitos numÃ©ricos vÃ¡lidos bajo algoritmo MÃ³dulo 10 Banxico.',
      'El monto a transferir debe ser mayor a $0.00 y menor o igual al saldo lÃ­quido disponible.',
      'Toda transacciÃ³n aprobada debe generar un folio de rastreo CEP Ãºnico y persistir en base de datos.'
    ]
  });
  ...
  const manejarSubidaReq = (e) => {
    ...
    if (file) {
      ...
      setRequerimiento({
        ...
        descripcion: `EspecificaciÃ³n cargada desde ${file.name} (${(file.size / 1024).toFixed(1)} KB). IDs generados con trazabilidad [${prefijoDinamico}].`,
        origen: 'Archivo local (' + file.name + ')',
        criterios: [
          'ValidaciÃ³n de reglas extraÃ­das del documento.',
  ...
  // GENERADOR DINÃMICO SEGÃšN NIVEL (JR: ~30 | MED: ~75 | SR: ~120)
  const generarSuitePorNivel = (prefijo, nivel) => {
    let distribucion = [
      ...
      { code: 'SQL', name: 'AuditorÃ­a SQL', casos: 2 }
    ]; // Junior ~30 casos
    ...
    if (nivel === 'semi') {
      distribucion = [
        ...
        { code: 'SQL', name: 'AuditorÃ­a SQL', casos: 7 }
      ...
      distribucion = [
        ...
        { code: 'SQL', name: 'AuditorÃ­a SQL', casos: 15 }
      ]; // Senior ~125 casos
    }
    ...
    distribucion.forEach(seccion => {
      for (let i = 1; i <= seccion.casos; i++) {
        ...
        suite.push({
          ...
          pasosArray: [
            { num: 1, accion: `Configurar entorno para prueba ${seccion.name} #${i} bajo requerimiento ${requerimiento.idHU}.`, data: `Param_${i}=Activo | Auth=JWT_Valid`, esperado: 'Entorno listo para validaciÃ³n.' },
            { num: 2, accion: `Ejecutar validaciÃ³n de ${seccion.name.toLowerCase()} en el mÃ³dulo de negocio.`, data: `Vector_Test=${seccion.code}_${i}`, esperado: 'Respuesta conforme a especificaciÃ³n tÃ©cnica.' },
            { num: 3, accion: 'Verificar registros de auditorÃ­a y persistencia en base de datos.', data: `Query: SELECT * FROM logs WHERE ref = '${idCaso}'`, esperado: 'Persistencia inmutable verificada.' }
          ],
          valores: {
            'ID_Caso': idCaso,
            'Modulo_Core': `${prefijo} / MÃ³dulo ${seccion.name}`,
            'Requerimiento_Asociado': requerimiento.idHU,
            'Descripcion_Escenario': `ValidaciÃ³n (${seccion.name}) #${i} basada en la especificaciÃ³n del requerimiento ${requerimiento.idHU}.`,
            'Tipo_Validacion': `QA ${nivel.toUpperCase()} - ${seccion.name}`,
            'Precondiciones': `Servicios core activos y parÃ¡metros cargados para ${idCaso}.`,
            'Pasos_Detallados': `1. Configurar datos ${i}.\n2. Ejecutar transacciÃ³n.\n3. Validar BD.`,
            'Valores_Entrada_TestData': `Dataset_${seccion.code}_${i}`,
            'Comportamiento_Esperado': `El sistema cumple con la regla de aceptaciÃ³n para ${seccion.name}.`,
            'Severidad': i <= 2 ? 'CrÃ­tica' : 'Alta',
  ...
  const [payloadJson, setPayloadJson] = useState(JSON.stringify({
    ...
    cliente: {
      ...
      empresa: "Servicios de AutomatizaciÃ³n & QA",
  ...
  const cambiarEvento = (e) => {
    ...
    if (val === 'lead') {
      ...
      setPayloadJson(JSON.stringify({
        ...
        descripcion: "Fallo en algoritmo de dÃ­gito verificador CLABE",
  ...
  const ejecutarFlujoN8n = () => {
    ...
    setTimeout(() => {
      ...
      setLogSalida({
        status: 200,
        mensaje: "Evento '" + (parsed.evento || 'HTTP_EVENT') + "' procesado exitosamente por la canalizaciÃ³n.",
  ...
  return (
    <section id="automatizaciones" className="max-w-6xl mx-auto px-4 py-16 w-full scroll-mt-24">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
          <div className="relative w-full max-w-2xl bg-slate-900 border border-cyan-500/60 rounded-3xl p-6 sm:p-8 shadow-2xl text-slate-100 space-y-5 max-h-[92vh] overflow-y-auto">
            ...
            <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl text-xs space-y-1">
              <span className="font-bold text-cyan-300 block text-[11px]">ðŸŽ¯ Precondiciones:</span>
            ...
            {!veredictoFinal ? (
              <div className="bg-slate-950 border border-cyan-500/40 rounded-2xl p-5 space-y-4 shadow-xl">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <span className="text-xs font-bold text-cyan-400 uppercase tracking-wider">
                    ðŸ‘‰ Paso {casoEnEjecucion.pasosArray[pasoActualIdx].num} en Progreso
                  </span>
                </div>

                <div className="space-y-3 text-xs">
                  <div>
                    <span className="text-slate-400 font-semibold block text-[11px]">ðŸ“‹ AcciÃ³n:</span>
                    <p className="text-white font-medium text-sm mt-0.5">{casoEnEjecucion.pasosArray[pasoActualIdx].accion}</p>
                  </div>
                  <div className="p-3 bg-slate-900 rounded-xl border border-slate-800">
                    <span className="text-cyan-300 font-semibold block text-[11px]">ðŸ§ª Datos de Prueba:</span>
                  ...
                  <div className="p-3 bg-emerald-950/40 rounded-xl border border-emerald-900/60">
                    <span className="text-emerald-400 font-semibold block text-[11px]">âœ… Resultado Esperado:</span>
              ...
              <div className={`p-6 rounded-2xl border text-center space-y-4 ${
                ...
                <div className="text-4xl">{veredictoFinal === 'PASSED' ? 'ðŸ†' : 'ðŸ›'}</div>
                <div>
                  <h4 className="text-lg font-extrabold">
                    {veredictoFinal === 'PASSED' ? 'Â¡Caso Ejecutado con Ã‰xito (PASS)!' : 'Defecto Detectado (FAIL)'}
                  </h4>
                </div>
                <div className="flex justify-center gap-3">
                  <button onClick={reiniciarEjecucion} className="bg-slate-800 text-white text-xs px-4 py-2 rounded-xl cursor-pointer">ðŸ”„ Re-ejecutar</button>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        {/* PESTAÃ‘A 1: MATRIZ QA CON LOS 3 PASOS */}
        {pestanaActiva === 'matriz' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 border-b border-slate-800 pb-5">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold">Archivo 3: Suite QA & MÃ©tricas</span>
                </div>
              ...
              <div className="space-y-5 text-xs">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-3">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400">Sube la especificaciÃ³n o historia de usuario que deseas probar.</p>
                  </div>
                </div>
                ...
                <div className="bg-slate-950 border-2 border-dashed border-slate-700 hover:border-emerald-500 rounded-2xl p-6 text-center space-y-3 transition">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-1">
                      Soporta: ImÃ¡genes (.png, .jpg), Documentos (.pdf, .docx), Excel (.xlsx) o Texto (.txt)
                    </p>
                  </div>
                  ...
                  {vistaPreviaReqImg && (
                    <div className="max-w-xs mx-auto p-2 bg-slate-900 rounded-xl border border-slate-800">
                      ...
                      <span className="text-[10px] text-emerald-400 block mt-1">âœ“ Imagen de requerimiento cargada</span>
                    </div>
                  )}
            ...
            <div className="mt-4 p-4 bg-gray-900 border border-gray-700 rounded-lg">
              <h3 className="text-sm font-semibold text-gray-200 mb-2">✍️ Redactar Requerimiento / Proyecto Manualmente</h3>
              <textarea
                ...
                placeholder="Escribe aquí los detalles de tu flujo o historia de usuario si no cuentas con un archivo..."
              ...
              <h3 className="text-sm font-semibold text-gray-200 mb-2">📌 Notas u Observaciones (Opcional)</h3>
              <textarea
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800 pb-6 mb-8">
              <div>
                <div className="inline-flex items-center gap-2 bg-cyan-950/80 border border-cyan-500/40 px-3 py-1 rounded-full text-xs font-semibold text-cyan-300 mb-2">
                  ...
                  <span>MÃ³dulo Funcional: AutomatizaciÃ³n con n8n</span>
                </div>
                <h2 className="text-2xl sm:text-3xl font-extrabold text-white">
                  OrquestaciÃ³n de Flujos & Webhooks en Vivo
                </h2>
                <p className="text-xs sm:text-sm text-slate-400 mt-1">
                  Prueba la recepciÃ³n de peticiones HTTP, validaciÃ³n de schemas JSON y despacho de alertas en tiempo real.
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  <option value="lead">ðŸ“¨ Captura de Lead / CotizaciÃ³n</option>
                  <option value="bug">ðŸ› Reporte de Defecto (Jira QA)</option>
                  <option value="api-health">ðŸ©º Health-Check API SPEI</option>
                </select>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-5 space-y-4">
                <div className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-3 text-xs">
                  <div>
                    <label className="block text-slate-400 font-semibold mb-1">MÃ©todo & Endpoint HTTP POST:</label>
                  ...
                  <div>
                    ...
                    ></textarea>
                    {errorJson && (
                      <p className="text-rose-400 text-[11px] font-semibold mt-1">âš ï¸ {errorJson}</p>
                    )}
                  </div>
                  ...
                  >
                    <span>{ejecutandoN8n ? 'âš™ï¸' : 'âš¡'}</span>
                ...
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-2xl font-mono text-xs text-slate-300 space-y-2">
                  <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                    <span className="text-emerald-400 font-bold text-[11px]">
                      âœ“ HTTP {logSalida.status} OK â€¢ Latencia: {logSalida.tiempoTotal}
                    </span>
